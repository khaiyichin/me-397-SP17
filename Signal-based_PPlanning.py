...
size = 5 # size of graph (number of nodes)
space = 5 # euclidean space in each dimension (max x,y,z coordinates)
...
class Ant(object):
    def __init__(self,starting_node_object,phero=Q):
    ...
    def cycle(self): # each cycle
        destination_node = size # the final node
...
def ACO_metaheuristic():
    ...
    ant_graph = initialize_graph(yaml_file='(5-3)nodes.yaml')
